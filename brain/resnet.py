...
model = resnet([2,2,2,2,2],input_dim=128,n_classes=n_classes)

x = torch.randn(1,3,128,128)
output = model(x)
print(f'model output shape: {output.shape[0],output.shape[1]}.')
...
with open('./res_brain_history_.pickle', 'wb') as handle:
    pickle.dump(history, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('./res_brain_prediction.pickle', 'wb') as handle:
